    ...
    if analysis["vulnerabilities"]:
        ...
        for vuln in analysis["vulnerabilities"]:
            report_lines.extend([
                ...
                f"Details: {vuln['details'][:200]}..." if len(vuln['details']) > 200 else f"Details: {vuln['details']}",
                ""
            ])
    else:
        report_lines.extend([
...
def main():
    ...
    parser.add_argument("--output-dir", default="./security_results", 
                       help="Directory to save security test results")
    parser.add_argument("--skip-auth-tests", action="store_true",
                       help="Skip authentication security tests")
    parser.add_argument("--skip-data-tests", action="store_true",
                       help="Skip data access security tests")
    parser.add_argument("--skip-url-tests", action="store_true",
                       help="Skip signed URL security tests")
