...
class TestSignedUrlSecurity(E2ETestBase):
    ...
    def create_mock_signed_url(self, user_id: str, filename: str, expiration_minutes: int = 60) -> str:
        ...
        return f"https://storage.googleapis.com/test-bucket/original/{user_id}/{filename}?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=test&X-Goog-Date=20240101T000000Z&X-Goog-Expires={expiration_minutes * 60}&X-Goog-SignedHeaders=host&X-Goog-Signature=test_signature_{expiration_time}"
    ...
    def test_signed_url_parameter_injection_prevention(self, test_users):
        ...
        def get_signed_url_side_effect(file_path, expiration=3600):
            ...
            if "?" in filename or "&" in filename:
                raise ValueError(f"Invalid filename: contains query parameters")
            if "%" in filename:
                raise ValueError(f"Invalid filename: contains URL encoding")
            if "\x00" in filename or "\r" in filename or "\n" in filename:
                raise ValueError(f"Invalid filename: contains control characters")
            if "'" in filename or '"' in filename:
                raise ValueError(f"Invalid filename: contains quote characters")
            if "<" in filename or ">" in filename:
                raise ValueError(f"Invalid filename: contains HTML characters")
