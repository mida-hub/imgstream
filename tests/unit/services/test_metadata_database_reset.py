...
class TestMetadataServiceDatabaseReset:
    ...
    def test_force_reload_from_gcs_successful_reset(self, mock_log_user_action):
        ...
        with patch.object(self.metadata_service, 'db_manager', mock_db_manager), \
             patch.object(self.metadata_service, 'ensure_local_database'):
    ...
    def test_force_reload_from_gcs_database_close_failure(self):
        ...
        assert result["success"] is True
        # Database manager should be reset to None
        assert self.metadata_service._db_manager is None
    ...
    def test_validate_database_integrity_valid_database(self):
        ...
        # Mock database operations for valid database
        mock_db_manager = MagicMock()
        mock_connection = MagicMock()
        mock_connection.__enter__ = MagicMock(return_value=mock_connection)
        mock_connection.__exit__ = MagicMock(return_value=None)
        
        # Mock query results for valid database
        mock_connection.execute.side_effect = [
            MagicMock(fetchone=lambda: ("photos",)),  # Table exists
            MagicMock(fetchone=lambda: (0,)),  # No orphaned records
            MagicMock(fetchall=lambda: []),  # No duplicates
            MagicMock(fetchone=lambda: (0,)),  # No invalid paths
            MagicMock(fetchone=lambda: (0,)),  # No future dates
        ]
    ...
    def test_validate_database_integrity_invalid_database(self):
        ...
        # Mock database operations for invalid database
        mock_db_manager = MagicMock()
        mock_connection = MagicMock()
        mock_connection.__enter__ = MagicMock(return_value=mock_connection)
        mock_connection.__exit__ = MagicMock(return_value=None)
        
        # Mock query results for invalid database
        mock_connection.execute.side_effect = [
            MagicMock(fetchone=lambda: None),  # Table doesn't exist
            MagicMock(fetchone=lambda: (5,)),  # 5 orphaned records
            MagicMock(fetchall=lambda: [("test.jpg", 2), ("another.jpg", 3)]),  # Duplicates
            MagicMock(fetchone=lambda: (2,)),  # 2 invalid paths
            MagicMock(fetchone=lambda: (1,)),  # 1 future date
        ]
...
class TestDatabaseResetIntegration:
    ...
    def test_complete_reset_workflow(self, mock_get_storage_service):
        ...
        with patch.object(metadata_service, 'ensure_local_database'), \
             patch.object(metadata_service, 'db_manager') as mock_db_manager:
