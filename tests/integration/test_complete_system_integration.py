...
class TestCompleteSystemIntegration:
    ...
    def test_complete_upload_workflow_with_collisions(self, mock_image_processor_class, 
        ...
        mock_metadata_service.check_filename_exists.side_effect = mock_check_filename_exists
        ...
        # Check overwrite events
        overwrite_events = [e for e in monitor.overwrite_events if e.user_id == self.user_id]
        assert len(overwrite_events) >= 1  # At least one overwrite operation
    ...
    def test_performance_under_load(self, mock_get_metadata_service):
        ...
        mock_service.check_multiple_filename_exists = MagicMock()
        mock_service.check_multiple_filename_exists.side_effect = lambda batch: {
            filename: mock_check_performance(filename)
            for filename in batch
            if mock_check_performance(filename) is not None
        }
        ...
        assert optimized_time < regular_time  # Optimized should be faster
        ...
        assert files_per_second_optimized > files_per_second_regular  # Optimized should be faster
    ...
    def test_cache_effectiveness(self, mock_get_metadata_service):
        ...
        assert first_call_count == 3  # 3 database calls for first request
        assert second_call_count == 3  # No additional calls for cached request
        assert third_call_count == 5  # 2 additional calls for new files
        
        # Check cache statistics
        final_stats = get_collision_cache_stats()
        assert final_stats["total_entries"] > initial_stats["total_entries"]
        assert final_stats["valid_entries"] >= 5  # At least 5 cached entries
    ...
    def test_error_recovery_integration(self, mock_get_metadata_service):
        ...
        mock_service.check_filename_exists.side_effect = Exception("Database error")
    ...
    def test_system_resource_cleanup(self):
        ...
        assert object_increase < 1000  # Should not create more than 1000 new objects
