...
class TestCollisionDetectionIntegration:
    ...
    def test_error_recovery_scenarios(self, mock_get_metadata_service):
        ...
        # Configure metadata service to fail initially, then succeed
        mock_get_metadata_service.return_value = self.mock_metadata_service
        
        call_count = 0
        def mock_check_with_failure(filename):
            nonlocal call_count
            call_count += 1
        ...
        self.mock_metadata_service.check_filename_exists.side_effect = mock_check_with_failure
        ...
        # Verify error handling logged appropriate events
        monitor = get_collision_monitor()
        # Should have logged the fallback collision detection
        assert len(monitor.collision_events) >= 1
    ...
    def test_database_consistency_during_overwrite(self, mock_image_processor_class, mock_get_storage_service,
        ...
        # Verify overwrite operation was logged
        monitor = get_collision_monitor()
        overwrite_events = monitor.overwrite_events
    ...
    def test_backward_compatibility_with_existing_photos(self):
        ...
        with patch('imgstream.utils.collision_detection.get_metadata_service') as mock_get_service:
            ...
            # Configure service to return old format photo
            mock_service.check_filename_exists.return_value = {
    ...
    def test_concurrent_collision_detection(self):
        ...
        with patch('imgstream.utils.collision_detection.get_metadata_service') as mock_get_service:
            ...
            call_count = 0
            def mock_check_with_race_condition(filename):
                nonlocal call_count
                call_count += 1
                ...
                if call_count == 1:
                    ...
                    existing_photo = self.create_existing_photo(filename, f"race_condition_{call_count}")
                    return {
                        "existing_photo": existing_photo,
                        "existing_file_info": {
                            "photo_id": existing_photo.id,
            ...
            mock_service.check_filename_exists.side_effect = mock_check_with_race_condition
            
            # First collision check
            result1 = check_filename_collisions(self.user_id, ["race_test.jpg"])
            assert len(result1) == 0  # No collision initially
            
            # Second collision check (simulating later check)
            result2 = check_filename_collisions(self.user_id, ["race_test.jpg"])
    ...
    def test_large_batch_processing(self):
        ...
        with patch('imgstream.utils.collision_detection.get_metadata_service') as mock_get_service:
            ...
            def mock_check_large_batch(filename):
            ...
            mock_service.check_filename_exists.side_effect = mock_check_large_batch
...
class TestDataIntegrity:
    ...
    def test_concurrent_overwrite_protection(self):
        ...
        with patch('imgstream.utils.collision_detection.get_metadata_service') as mock_get_service:
            ...
            result1 = check_filename_collisions(self.user_id, ["concurrent_test.jpg"])
            result2 = check_filename_collisions(self.user_id, ["concurrent_test.jpg"])
