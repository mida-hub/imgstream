...
class MetricsCollector:
        ...
        try:
            ...
            with self._lock:
                ...
                self.performance_metrics.max_response_time = max(
                    self.performance_metrics.max_response_time, duration
                )
                self.performance_metrics.min_response_time = min(
                    self.performance_metrics.min_response_time, duration
                )
    ...
    def record_error(self, error_type: str, labels: Optional[Dict[str, str]] = None):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            avg_response_time = (
                ...
                max(self.performance_metrics.request_count, 1)
            )
            ...
            return {
                ...
                "min_response_time": (
                    ...
                    if self.performance_metrics.min_response_time != float('inf') else 0
                ),
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        for check_name, check_result in health_status["checks"].items():
            self.metrics.record_gauge(
                "health_check_status",
                1 if check_result["healthy"] else 0,
                {"check": check_name}
            )
...
def monitor_function(operation_name: str = None, labels: Optional[Dict[str, str]] = None):
...
def record_business_event(event_name: str, value: float = 1.0, 
                         labels: Optional[Dict[str, str]] = None):
...
def monitor_upload(func: Callable) -> Callable:
    ...
    return monitor_function("file_upload", {"operation": "upload"})(func)


def monitor_auth(func: Callable) -> Callable:
    """Monitor authentication operations."""
    return monitor_function("authentication", {"operation": "auth"})(func)


def monitor_database(func: Callable) -> Callable:
    """Monitor database operations."""
    return monitor_function("database_operation", {"operation": "db"})(func)
