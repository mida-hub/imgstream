...
def render_performance_dashboard() -> None:
    ...
    with col1:
        ...
        st.metric(
            "Expired Entries", cache_stats.get("expired_entries", 0), help="Number of cache entries that have expired"
        )

        st.metric(
            "Cache TTL", f"{cache_stats.get('ttl_seconds', 0)}s", help="Time-to-live for cache entries in seconds"
        )
    ...
    with col2:
        ...
        if total_entries > 0:
            ...
            st.metric("Cache Hit Rate", f"{hit_rate:.1f}%", help="Percentage of cache entries that are still valid")
        else:
    ...
    with tab1:
        st.line_chart(chart_data["response_time"], use_container_width=True, height=300)
        st.caption("Response time trend over the last 24 hours")

    with tab2:
        st.line_chart(chart_data["throughput"], use_container_width=True, height=300)
        st.caption("Files processed per second over the last 24 hours")

    with tab3:
        st.line_chart(chart_data["error_rate"], use_container_width=True, height=300)
...
def _get_mock_chart_data() -> dict[str, Any]:
    ...
    return {
        "response_time": pd.DataFrame({"Response Time (s)": response_times}, index=hours),
        "throughput": pd.DataFrame({"Files/Second": throughput}, index=hours),
        "error_rate": pd.DataFrame({"Error Rate (%)": error_rates}, index=hours),
    }
...
def render_performance_summary_widget() -> None:
    ...
    with col1:
        st.metric("Cache Entries", cache_stats.get("valid_entries", 0), help="Active cache entries")

    with col2:
        st.metric("Avg Time", f"{performance_data['avg_response_time']:.3f}s", help="Average response time")
