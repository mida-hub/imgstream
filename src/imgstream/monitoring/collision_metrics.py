...
class CollisionMetricsCollector:
    ...
    def log_collision_event(self, user_id: str, event_type: str, filename: str, **details) -> None:
        """Log a collision detection event."""
        event = CollisionEvent(
            timestamp=datetime.now(), user_id=user_id, event_type=event_type, filename=filename, details=details
        )
        ...
        # Log user action
        log_user_action(user_id, f"collision_{event_type}", filename=filename, **details)
    ...
    def track_user_decision(
        self, user_id: str, decision: str, decision_time_seconds: float = 0.0  # 'overwrite', 'skip', 'pending'
    ) -> None:
            ...
            if decision_time_seconds > 0:
                ...
                if total_decisions > 0:
                    ...
                    pattern.avg_decision_time_seconds = (
                        current_avg * (total_decisions - 1) + decision_time_seconds
                    ) / total_decisions
        ...
        logger.info(
            ...
            overwrite_rate=(
                pattern.overwrite_decisions / pattern.total_collisions if pattern.total_collisions > 0 else 0
            ),
        )
        ...
        **additional_metrics,
        ...
        with self._lock:
            ...
            if hourly_stat["avg_response_time"] == 0:
                ...
                hourly_stat["avg_response_time"] = (hourly_stat["avg_response_time"] + duration_ms) / 2
    ...
    def get_collision_statistics(self, hours: int = 24) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_events = [event for event in self.collision_events if event.timestamp >= cutoff_time]

            recent_overwrites = [metric for metric in self.overwrite_metrics if metric.timestamp >= cutoff_time]
    ...
    def get_user_decision_patterns(self, user_id: str | None = None) -> dict[str, Any]:
            ...
            if user_id:
                ...
                if pattern:
                    return {
                        ...
                        "overwrite_rate": (
                            pattern.overwrite_decisions / pattern.total_collisions
                            if pattern.total_collisions > 0
                            else 0
                        ),
                        "skip_rate": (
                            pattern.skip_decisions / pattern.total_collisions if pattern.total_collisions > 0 else 0
                        ),
    ...
    def get_hourly_trends(self, hours: int = 24) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_stats = {hour: stats for hour, stats in self.hourly_stats.items() if hour >= cutoff_time}
            ...
            return {
                ...
                "summary": {
                    ...
                    "peak_collision_hour": (
                        max(hourly_data, key=lambda x: x["collision_count"]) if hourly_data else None
                    ),
    ...
    def _calculate_aggregate_user_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_users_last_24h": len(
                [p for p in patterns if p.last_activity and p.last_activity >= datetime.now() - timedelta(hours=24)]
            ),
        }
...
def log_batch_collision_check(
    user_id: str, file_count: int, collisions_found: int, duration_ms: float, **details
) -> None:
    ...
    _metrics_collector.log_performance_metrics("batch_collision_check", duration_ms, file_count, **details)
