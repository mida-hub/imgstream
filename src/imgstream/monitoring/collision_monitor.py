...
class CollisionMonitor:
    ...
    def log_collision_detected(
        self, user_id: str, filename: str, existing_photo_id: str, processing_time_ms: float = None, **metadata
    ) -> None:
        """Log a collision detection event."""
        ...
        logger.info("collision_detected", **event.to_dict())
    ...
    def log_collision_resolved(
        self, user_id: str, filename: str, user_decision: str, decision_time_ms: float = None, **metadata
    ) -> None:
        ...
        logger.info("collision_resolved", **event.to_dict())

        log_user_action(
            user_id, "collision_resolved", filename=filename, decision=user_decision, decision_time_ms=decision_time_ms
        )
    ...
    def log_overwrite_operation(
        ...
        **metadata,
        ...
        logger.info("overwrite_operation", **event.to_dict())
    ...
    def log_user_decision(
        self, user_id: str, filename: str, decision: str, decision_time_ms: float = None, **collision_context
    ) -> None:
        ...
        logger.info("user_decision", **event.to_dict())

        log_user_action(
            user_id, "user_decision", filename=filename, decision=decision, decision_time_ms=decision_time_ms
        )

    def log_batch_collision_detection(
        self,
        user_id: str,
        filenames: list[str],
        ...
        **metadata,
        ...
        logger.info("batch_collision_detection", user_id=user_id, **batch_metadata)

        log_user_action(user_id, "batch_collision_detection", **batch_metadata)

    def get_collision_statistics(self, user_id: str = None, time_window: timedelta = None) -> dict[str, Any]:
        ...
        return {
            ...
            "user_decision_metrics": {
                ...
                "overwrite_rate": (
                    decision_counts.get("overwrite", 0) / len(filtered_decisions) if filtered_decisions else 0
                ),
            ...
            "overwrite_metrics": {
                ...
                "success_rate": (
                    overwrite_counts.get("success", 0) / len(filtered_overwrites) if filtered_overwrites else 0
                ),
                "fallback_rate": (
                    overwrite_counts.get("fallback", 0) / len(filtered_overwrites) if filtered_overwrites else 0
                ),
    ...
    def get_user_behavior_patterns(self, user_id: str, time_window: timedelta = None) -> dict[str, Any]:
        ...
        user_decisions = [
            event for event in self.user_decision_events if event.user_id == user_id and event.timestamp >= cutoff_time
        ]
    ...
    def export_events(self, format: str = "json", user_id: str = None, time_window: timedelta = None) -> str:
...
def log_collision_detected(
    user_id: str, filename: str, existing_photo_id: str, processing_time_ms: float = None, **metadata
) -> None:
    """Log a collision detection event."""
    _collision_monitor.log_collision_detected(user_id, filename, existing_photo_id, processing_time_ms, **metadata)


def log_collision_resolved(
    user_id: str, filename: str, user_decision: str, decision_time_ms: float = None, **metadata
) -> None:
    """Log a collision resolution event."""
    _collision_monitor.log_collision_resolved(user_id, filename, user_decision, decision_time_ms, **metadata)
...
def log_overwrite_operation(
    ...
    **metadata,
    ...
    """Log an overwrite operation event."""
    _collision_monitor.log_overwrite_operation(
        user_id,
        filename,
        original_photo_id,
        operation_type,
...
def log_user_decision(
    user_id: str, filename: str, decision: str, decision_time_ms: float = None, **collision_context
) -> None:
    """Log a user decision event."""
    _collision_monitor.log_user_decision(user_id, filename, decision, decision_time_ms, **collision_context)
...
def log_batch_collision_detection(
    ...
    **metadata,
    ...
    """Log batch collision detection metrics."""
    _collision_monitor.log_batch_collision_detection(
        user_id, filenames, collisions_found, processing_time_ms, cache_hit_rate, **metadata
    )
